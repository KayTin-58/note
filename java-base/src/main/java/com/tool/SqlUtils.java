...
public final class SqlUtils {
     ...
     * @return sql
     */
    public static String structureCreateSql(DataBaseType dataType, String tableName, List<Column> columns, Boolean ifPartition, List<Column> partitions, String rowFormat) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            switch (dataType) {
                case HIVE:
                    sql.append(String.format(HIVE_COLUMN_BASE,column.getColName(),typeOfData,APO_STROPHE+column.getRemarks()+APO_STROPHE));
                    break;
                case MYSQL:
        ...
        if(ifPartition) {
            ...
            partitions.forEach(s ->
               sql.append(s.getColName()).append(SPACE).append(s.getTypeName())
            );
     ...
     * @return sql
     */
    public static String structureInsertSql(DataBaseType dataBaseType, String schema, String tableName, List<Tuple<String, String>> values) {
        StringBuilder sql = new StringBuilder();
