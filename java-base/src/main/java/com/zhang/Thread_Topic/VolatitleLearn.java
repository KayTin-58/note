...
/**
 * 内存 ：int num = 10; 线程1： 缓存：int mun = 10; 线程2： 缓存：int num = 10;
 *
 * 非volatitle 变量： 线程1修改 num = 100;
 *
 * 内存 ：int num = 10; 线程1： 缓存：int mun = 100; 线程2： 缓存：int num = 10;
 *
 *
 * 此时线程2读到的num是缓存中的num值 num = 10
 *
 * volatitle 变量： 线程1修改 num = 100;
 *
 * 内存 ：int num = 100; 线程1： 缓存：int mun = 100; 线程2： 缓存：int num = 100;
 *
...
public class VolatitleLearn {
    ...
    /**
     * 以下代码还是会出现 counter 出现重复值 原因在于 counter++ 不是一个原子操作
     */
